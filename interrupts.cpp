    ...
    for(size_t i = 0; i < trace_file.size(); i++) {
        ...
        if(activity == "CPU") { //As per Assignment 1
            ...
            if (!child_alloc_ok) {
                ...
                execution += std::to_string(current_time) + ", 0, no free partition for child, continuing with parent\n";
                ...
                system_status += print_PCB(current, snapshot_q) + "\n";
            } else {
                ...
                
                {
                    std::vector<PCB> snapshot_q = wait_queue;
                    system_status += "time: " + std::to_string(current_time) + "; current trace: FORK, " + std::to_string(duration_intr) + "\n";
                    system_status += print_PCB(child, snapshot_q) + "\n";
                }
            }
            ...
            if (!child_trace.empty() && child_alloc_ok) {
                auto [child_exec, child_stat, child_time] =
                ...
                wait_queue.erase(
                    std::remove_if(wait_queue.begin(), wait_queue.end(), [&](const PCB& p){ return p.PID == current.PID; }),
                    wait_queue.end());
                }
            ...
            std::vector<PCB> snapshot_q = wait_queue;
            snapshot_q.erase(
                std::remove_if(snapshot_q.begin(), snapshot_q.end(), [&](const PCB& p){ return p.PID == current.PID; }),
                snapshot_q.end());
